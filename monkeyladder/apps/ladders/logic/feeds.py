...
def public_ladder_feed(user=None, order='-created', size=25):
    ...
    """
    ladders = Ladder.objects.filter(is_private=False, is_active=True).order_by(order)[:size]
...
def watched_ladder_feed(user, order='-created', size=25, include_watchers=False, **filters):
    ...
    if user.is_authenticated():
        watchers = user.watcher_set.filter(ladder__is_active=True, **filters).order_by(order)[:size]
        if include_watchers:
